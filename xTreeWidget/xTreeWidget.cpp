    ...
    : QTreeWidget(parent)
{
    ...
    QAction* del = new QAction(QIcon(":/PahpadsInstructor/Resources/buttons/remove.png"), tr("&Delete"), this);
    menuActions.append(del);
}
...
void xTreeWidget::saveToXML(const QString path) const
{
    ...
    if (file.open(QIODevice::ReadWrite))
    {
        ...
        _toXML = [&](QTreeWidgetItem* itm, int level)
        {
            ...
            const QString xmlStr = nInfo.data->toString();
            QString ret;
            ...
            if (!xmlStr.isEmpty())
            {
                ...
                ret += QString ("<%1 %2 ").arg(nInfo.type).arg(treeStateStr)+xmlStr+((itm->childCount()>0)?" >":" />")+"\n";
